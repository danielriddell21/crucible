...
// resolution. A zero Frames records until the run decides to stop; pre-set it
// to change the flag's default.
func (o *Options) AddPacedFlags(fs *pflag.FlagSet) {
	fs.StringVar(&o.Path, "record", o.Path, "record the run to this GIF path, then exit")
	fs.IntVar(&o.Frames, "record-frames", o.Frames, "frames to capture before exiting (0 records the whole run)")
}
