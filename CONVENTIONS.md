...
`synth`, `record`, `hub`, `worldgen`, `level`, `raycast`, `camera`), no
`cmd/` and no `internal/`.
...
- **App types stay in the apps.** Packages that relay or store app-defined
  ...
  message, event, and cue vocabularies belong to each game. The one
  engine-owned vocabulary is `level`'s spatial one — tiles, heights, half
  walls, lifts, doors, vents — kept deliberately general (`TileSwitch` is any
  interactable, `TileCover` any hiding spot) so world features built for one
  game are reusable in the next; gameplay meaning stays app-side.
...
- **CLI entrypoint.** The command tree, the root command (silenced
  usage/errors, `--version`, `completion` registered), and the entrypoint
  all stay in each repo's `internal/cli` with a thin `cmd/<name>/main.go` —
  crucible is engine code and stays out of the CLI. Do not add a bespoke
  `version` subcommand.
